...
const FormularioPaciente = () => {
    ...
    useEffect(() => {
        ...
        if (doctorGuardado) {
            ...
            setEspecialidad(doctor.especialidad || 'Medicina General'); 
        }
    }, []);

    const handleTextChange = (valor, setter) => {
        const soloLetras = /^[a-zA-ZáéíóúÁÉÍÓÚñÑ ]*$/;
        if (soloLetras.test(valor) && valor.length <= 30) { setter(valor); }
    };
    ...
    const esTextoCoherente = (texto) => {
        if (/(.)\1{2,}/.test(texto)) return false; 
        if (/[bcdfghjklmnpqrstvwxyz]{5,}/i.test(texto)) return false; 
        return true;
    };
    ...
    const guardarPaciente = (e) => {
        ...
        if (!esTextoCoherente(nombre) || !esTextoCoherente(apellidoPaterno)) {
            return Swal.fire('Nombre Inválido', 'Por favor, ingrese nombres reales y coherentes.', 'warning');
        }
        if (especialidad === 'Pediatría' && (pesoNacer < 0.5 || pesoNacer > 6.0)) return Swal.fire('Peso Inválido', 'Entre 0.5 y 6.0.', 'warning');
        if (especialidad === 'Cardiología' && (frecuenciaCardiaca < 30 || frecuenciaCardiaca > 250)) return Swal.fire('Frecuencia Inválida', 'Rango: 30 a 250.', 'warning');
        if (especialidad === 'Traumatología' && !zonaLesion) return Swal.fire('Dato Incompleto', 'Seleccione una zona.', 'warning');
        if (especialidad === 'Neurología' && (escalaGlasgow < 3 || escalaGlasgow > 15)) return Swal.fire('Rango Inválido', 'Glasgow entre 3 y 15.', 'warning');
        if (especialidad === 'Medicina General' && (temperatura < 34 || temperatura > 43)) return Swal.fire('Rango Inválido', 'Temperatura fuera de rango.', 'warning');
        if (especialidad === 'Cardiología' && presionArterial && !/^\d{2,3}\/\d{2,3}$/.test(presionArterial)) return Swal.fire('Formato Incorrecto', 'Ej: 120/80', 'warning');
        ...
        // 🚀 FIX DEFINITIVO 1: Fecha blindada a 19 caracteres (Ej: "2026-04-26 13:45:00")
        const ahora = new Date();
        const fechaSegura = ahora.toISOString().slice(0, 19).replace('T', ' ');

        let detallesEspecialidad = "";
        switch (especialidad) {
            case 'Cardiología': detallesEspecialidad = `Cardiología -> Presión: ${presionArterial}, Frec. Card: ${frecuenciaCardiaca}, Colesterol: ${colesterol}`; break;
            case 'Pediatría': detallesEspecialidad = `Pediatría -> Peso Nacer: ${pesoNacer}kg, Tutor: ${nombreTutor}, Vacunas: ${vacunasAlDia}`; break;
            case 'Neurología': detallesEspecialidad = `Neurología -> Glasgow: ${escalaGlasgow}, Cefaleas: ${frecuenciaCefaleas}`; break;
            case 'Traumatología': detallesEspecialidad = `Traumatología -> Zona: ${zonaLesion}, Dolor: ${nivelDolor}/10, Motivo: ${motivoConsulta}`; break;
            case 'Medicina General': detallesEspecialidad = `Medicina General -> Temp: ${temperatura}°C, Motivo: ${motivoConsulta}`; break;
            default: break;
        }

        // 🚀 FIX DEFINITIVO 2: Paquete seguro y con ID
        let nuevoPaciente = { 
            id: window.crypto.randomUUID(),
            nombre: nombre.trim().substring(0, 140), 
            apellidoPaterno: apellidoPaterno.trim().substring(0, 20), 
            apellidoMaterno: apellidoMaterno.trim().substring(0, 20), 
            dni: dni, 
            edad: parseInt(edad), 
            sexo: sexo, 
            tipoSangre: tipoSangre, 
            alergiasConocidas: alergiasConocidas || "Ninguna",
        ...
        PacienteService.registrarPaciente(nuevoPaciente)
            .then(() => {
                Swal.fire({
                    ...
                    html: `Paciente guardado en la base de datos.<br><b>Cama:</b> ${numeroCamaGenerado}<br><b>HC:</b> ${historiaClinicaGenerada}`,
                    icon: 'success',
                    confirmButtonColor: '#1A365D'
                }).then(() => { window.location.reload(); });
            })
            .catch((err) => {
                // 🚀 FIX DEFINITIVO 3: Mostrar el error REAL para no adivinar
                console.error("Error devuelto por el servidor:", err);
                const errorMensaje = err.response?.data?.message || err.response?.data || err.message || "Error desconocido";
                Swal.fire('Error de Base de Datos', `Detalle: ${typeof errorMensaje === 'string' ? errorMensaje : JSON.stringify(errorMensaje)}`, 'error');
                setIsLoading(false); 
            });
    };
    ...
    return (
        <div style={{ backgroundColor: '#F1F5F9', padding: '40px 0' }}>
            <div className="animacion-entrada" style={{ margin: '0 auto', width: '100%', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 15px 35px rgba(26, 54, 93, 0.12)', backgroundColor: '#FFFFFF' }}>
                        ...
                        <div className="section-box" style={{ backgroundColor: '#F0F9FF', borderColor: '#00A8CC' }}> 
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px' }}> 
                                <div>
                                    ...
                                    <input className="input-clinico" type="text" placeholder="Ej: 120/80" value={presionArterial} onChange={(e)=>setPresionArterial(e.target.value)} style={inputBaseStyle} disabled={isLoading} />
                                </div> 
                                <div><label style={labelStyle}>Frecuencia (bpm)</label><input className="input-clinico" type="number" placeholder="Ej: 75" value={frecuenciaCardiaca} onChange={(e)=>setFrecuenciaCardiaca(e.target.value)} style={inputBaseStyle} disabled={isLoading} /></div> 
                    ...
                    {especialidad === 'Traumatología' && ( 
                        <div style={{ padding: '25px', borderRadius: '12px', background: '#FFF7ED', borderLeft: '6px solid #ED8936', marginTop: '20px' }}> 
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}> 
                                <div>
                                    ...
                                    <select className="input-clinico" style={{ width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #CBD5E1', background: 'white' }} value={zonaLesion} onChange={(e)=>setZonaLesion(e.target.value)} required disabled={isLoading}>
                                        <option value="">-- Seleccione --</option><option value="Hombro">Hombro</option><option value="Brazo / Húmero">Brazo</option><option value="Rodilla">Rodilla</option>
                                    </select>
                                </div> 
                            ...
                            <div style={{ marginTop: '20px' }}>
                                <label style={labelStyle}>Detalles Clínicos</label>
                                <textarea className="input-clinico" style={{ ...inputBaseStyle, height: '80px', resize: 'none' }} value={motivoConsulta} onChange={(e) => setMotivoConsulta(e.target.value)} placeholder="Ej: Paciente presenta dolor punzante tras caída..." required disabled={isLoading} />
                            </div>
                        </div> 
                    )}
